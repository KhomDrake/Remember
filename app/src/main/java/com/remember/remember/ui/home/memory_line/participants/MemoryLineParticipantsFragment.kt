...
class MemoryLineParticipantsFragment : Fragment(R.layout.app_fragment_memory_line_participants) {

    private val memoryLineParticipantsViewModel: MemoryLineParticipantsViewModel by viewModel()
    private val navArgs: MemoryLineParticipantsFragmentArgs by navArgs()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        memoryLineParticipantsViewModel.setIdMemoryLine(navArgs.idMemoryLine)
    ...
    private fun loadParticipants() {
        memoryLineParticipantsViewModel.participants()
            .observe(viewLifecycleOwner) {
                data {
                    setupParticipants(it.results)
                    viewStateMachine.changeState(if (it.results.count() == 0) stateEmpty else stateData)
                }
                showLoading {
